...
CNexDomeV3::CNexDomeV3()
{
    ...
    fprintf(Logfile, "[%s] [CNexDomeV3::CNexDomeV3] Version %3.2f build 2019_11_15_2100.\n", timestamp, DRIVER_VERSION);
...
int CNexDomeV3::domeCommand(const char *pszCmd, char *pszResult, int nResultMaxLen)
{
    ...
    while(true) {
        if(nb_timeout>2) { // durring a movement we get a lot of extra stuff in there
            nErr = ERR_RXTIMEOUT;
            return nErr;
        }
        // read response
...
int CNexDomeV3::getShutterState(int &nState)
{
	...
	if(!nOpen && !nClosed && m_nCurrentShutterCmd != IDLE) {
        ...
        snprintf(szBuf, SERIAL_BUFFER_SIZE, "@PWS,%d\r\n", 0);
...
bool CNexDomeV3::isDomeAtHome()
{
    ...
    do {
        ...
        timestamp = asctime(localtime(&ltime));
        timestamp[strlen(timestamp) - 1] = 0;
        fprintf(Logfile, "[%s] [CNexDomeV3::isDomeAtHome] response = %s\n", timestamp, szResp);
        fflush(Logfile);
    #endif
        if(nErr) {
            return false;
...
int CNexDomeV3::openShutter()
{
    ...
    memcpy(szBuf, szResp, SERIAL_BUFFER_SIZE);
    m_bDomeIsMoving = false;
    while(!strstr(szBuf, "OPS") && nb_timeout < 3) {
        nErr = processResponse(szBuf, szResp, SERIAL_BUFFER_SIZE);
        if(strstr(szResp, "OPS") ) {
            m_bDomeIsMoving = true;
            if(nErr == CMD_PROC_DONE)
                nErr = PLUGIN_OK;
            break;
...
int CNexDomeV3::closeShutter()
{
    ...
    memcpy(szBuf, szResp, SERIAL_BUFFER_SIZE);
    m_bDomeIsMoving = false;
    while(!strstr(szBuf, "CLS") && nb_timeout < 3) {
        nErr = processResponse(szBuf, szResp, SERIAL_BUFFER_SIZE);
